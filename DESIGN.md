...
- **"Map-TV" price boards = maps in item frames.** A `MapRenderer` draws live **text** (commodity name + current price + trend, over a small price-history line) directly onto the framed map, refreshed on a timer and tiled across a `cols×rows` wall — everything is painted on the map surface, so a map-TV **never spawns any entity** (no floating items, no air-holograms). For the full interactive chart the board is **right-click-interactive → it opens the live web-dashboard Chart.js graph in the player's browser** (`displays.maptv.dashboard_url`, deep-linked `?item=<id>`), reusing the TV block's clickable-link mechanism — zero server-side chart rendering. *(A v0.16.1 experiment that pinned a spinning `ItemDisplay` of the commodity to the board face and reformatted the text was reverted — the display entities leaked and stranded in-world; `/hcm display cleanup` despawns any such strays, scoped to the plugin's own map-TV-tagged displays.)*
