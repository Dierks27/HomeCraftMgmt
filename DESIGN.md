# HomeCraft Management — Plugin Design Specification (v15)
...
- **Loot Boxes / Crates:** spend tokens to open a themed crate for a **weighted-random reward** — cash, items, or (rarely) a **Mini**. A `mini` reward names a fixed id **or a `tag:` pool** (rarity-weighted) and **mints a finished, graded Mini directly** (grade + Shiny rolled with the Printer's odds) with a found-broadcast ("… found a [Mini] in a crate!"). **Cap-aware:** a Mini prize mints against its cap and stops appearing once minted out. Reuses the wild-drop weighting engine + rarity/glint visuals.
