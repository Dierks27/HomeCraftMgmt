...
- **"Map-TV" price boards = maps in item frames.** A `MapRenderer` draws live **text** onto map items (refreshed on a timer), tiled into a big-screen wall. The single-commodity map-TV renders a clean, sign-style **text board** — commodity name, a big price, a hand-drawn green ▲ / red ▼ trend triangle, and stock. The earlier price-history line graph was removed as unreliable (painting a chart on a map never worked across many rebuilds); **text on a map is the dependable path**, and for the actual chart the board is **right-click-interactive → it opens the live web-dashboard Chart.js graph in the player's browser** (`displays.maptv.dashboard_url`, deep-linked `?item=<id>`), reusing the TV block's clickable-link mechanism — zero server-side chart rendering. The commodity's **item floats pinned to the board face** (a reused `ItemDisplay`, hologram-style but mounted on the screen; `displays.maptv.show_item/item_scale/spin`).
